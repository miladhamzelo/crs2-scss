...
import {BubbleChartModel, VanDashboardModel} from './model/van-dashboard.model';
...
export class VanDashboardComponent implements OnInit {
    ...
    constructor(private http: HttpClient, private logger: NGXLogger,
                private appResizeService: AppResizeService, private titlecasePipe: TitleCasePipe) {
        this.appResizeService.appResized$.subscribe(isResized => this.onAppResize(isResized));
    ...
    filterDealers(name: string) {
        return name ? this.activeVanDealers.filter(dealer =>
    ...
    getAllActiveDealers() {
        ...
        this.http
            .get<DealerInfoModel[]>(url)
            .subscribe(
                data => {
                    this.activeVanDealers = data;
                    this.filteredDealers = this.dealerCtrl.valueChanges
                        .startWith(null)
                        .map(dealer => dealer && typeof dealer === 'object' ? dealer.dealerName : dealer)
                    ...
                    this.dashboardHeader += ' (' + this.monthYearInfo[0].monthYearDesc + ')';
    ...
    setDataForBubbleChart(kpiName: string) {
        for (const kpi of this.kpiInfo) {
            if (kpi.kpi === kpiName) {
                ...
                this.bubbleChart.series[0].setData(this.bubbleChartData, true, true, true);
                break;
            }
        }
    }
    ...
    setActlandObjForLastSixMonthsForGivenKpi(kpiName: string) {
        for (const kpi of this.kpiInfo) {
            if (kpi.kpi === kpiName) {
                ...
                this.barChart.series[0].setData(this.actForLastSixMonths, true, true, true);
                this.barChart.series[1].setData(this.objForLastSixMonths, true, true, true);
                break;
            }
        }
    }
    ...
    setBubbleChartInfo() {
        this.bubbleChart.setTitle({
            text: 'Dealer Performance - ' + this.titlecasePipe.transform(this.selectedDealerInfo.regionName) + ' Region'
        }, null);
    }
    ...
    getKpiInformation(): void {
        ...
        if (this.fetchMockData) {
            ...
            this.http
                .post<VanDashboardModel>(urlStr, body)
                .subscribe(
                    data => {
                        this.kpiInfo = data.kpiInfo;
                        this.setActlandObjForLastSixMonthsForGivenKpi(this.kpiInfo[0].kpi);
                        ...
                        this.dashboardHeader = this.selectedDealerInfo.dealerName;
                        this.dashboardHeader += ' (' + this.selectedMonthAndYear.monthYearDesc + ')';
                        this.setBubbleChartInfo();
                        this.setDataForBubbleChart(this.kpiInfo[0].kpi);
                    },
                    (err: HttpErrorResponse) => {
